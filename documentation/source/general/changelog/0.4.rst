...
For remote backend queries, Qrisp now uses the network interface developed in the `sequence project <https://sequence.de/>`_. This project aims to build a uniform, open-source quantum cloud infrastructure. Note that specific backend vendors like IBMQuantum can still be called via :ref:`VirtualBackends <VirtualBackend>`.
...
* Fixed a bug that caused problems during circuit visualization on some platforms.
