...
The :func:`GQSP angle computation <qrisp.gqsp.gqsp_angles>` runs efficiently using the inverse non-linear Fourier transform and spectral factorization. All of these integrate tightly with the :ref:`BlockEncoding <BlockEncoding>` class via its ``.poly``, ``.inv``, and ``.sim`` methods.
