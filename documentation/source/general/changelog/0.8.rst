...
The :ref:`BlockEncoding <BlockEncoding>` class is the central abstraction for Quantum Linear Algebra in Qrisp. It embeds a non-unitary operator $A$ into the upper-left block of a larger unitary $U_A$ and provides an intuitive, NumPy-like interface: users write matrix expressions - addition, multiplication, inversion, polynomial transformations - and Qrisp compiles them into gate sequences automatically.

Block encodings can be constructed from a variety of sources:

* A :ref:`QubitOperator <QubitOperator>` or :ref:`FermionicOperator <FermionicOperator>` via :func:`BlockEncoding.from_operator <qrisp.block_encodings.BlockEncoding.from_operator>`.
* A NumPy array or sparse matrix via :func:`BlockEncoding.from_array <qrisp.block_encodings.BlockEncoding.from_array>`.
* A custom Linear Combination of Unitaries via :func:`BlockEncoding.from_lcu <qrisp.block_encodings.BlockEncoding.from_lcu>`.

Once constructed, block encodings support a rich set of operations:

* **Matrix arithmetic**: ``+``, ``-``, ``*``, ``@`` (matrix product), Kronecker product.
...
* **Resource estimation**: :func:`resources <qrisp.block_encodings.BlockEncoding.resources>` for gate count and depth analysis.

For a hands-on introduction, check out the :ref:`Block Encoding tutorial <BE101>`.

Scalable Shor's Algorithm
-------------------------

Building on the modular arithmetic introduced in Qrisp 0.4, this release extends the :ref:`Shor <shors_alg>` implementation to **utility-scale** by leveraging Jasp for compilation and resource estimation. Key ingredients include:
...
* **Jasp-compatible Montgomery arithmetic**: The full Montgomery multiplication pipeline - reduction, constant-quantum multiplication, quantum-quantum multiplication - has been reimplemented for Jasp tracing using ``jrange``, ``BigInteger``, and the :ref:`Gidney adder <gidney_adder>`.
...
The :func:`GQSP angle computation <qrisp.gqsp.gqsp_angles>` runs in $\mathcal{O}(d \log d)$ using the inverse non-linear Fourier transform and spectral factorization. All of these integrate tightly with the :ref:`BlockEncoding <BlockEncoding>` class via its ``.poly()``, ``.inv()``, and ``.sim()`` methods.
...
The `Childs-Kothari-Somma <https://arxiv.org/abs/1511.02306>`_ (CKS) algorithm for solving quantum linear systems has been implemented as a standalone :ref:`algorithm <CKS>`. It uses LCU with qubitization and a Chebyshev polynomial approximation of $1/x$ within a Repeat-Until-Success loop. It is also available as the :func:`BlockEncoding.inv <qrisp.block_encodings.BlockEncoding.inv>` method for a more integrated workflow.
...
* :doc:`Shor's algorithm at scale </general/tutorial/BigInteger>` - Resource estimation for Shor's algorithm at 128–2048 bit key sizes.
* :doc:`Fault-Tolerant Compilation </general/tutorial/FT_compilation>` - T-gate optimization and Clifford+T synthesis for fault-tolerant devices.
