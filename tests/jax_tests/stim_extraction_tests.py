...
def test_basic_parity_detector():
    ...
    num_detectors = (
        ...
        if hasattr(stim_circuit, "num_detectors")
        else len([i for i in stim_circuit if i.name == "DETECTOR"])
    )
...
def test_observable_merging():
    ...
    obs_lines = [
        l for l in str(stim_circuit).splitlines() if "OBSERVABLE_INCLUDE(2)" in l
    ]
...
def test_detector_order_invalid_value():
    ...
    with pytest.raises(
        ValueError, match="detector_order must be 'chronological' or 'return_order'"
    ):
...
def test_detector_order_partial_detectors():
    ...
    with pytest.raises(
        ValueError, match="Circuit contains 5 detectors, but permutation has length 3"
    ):
        partial_detectors()
...
def test_detector_order_repetition_code():
    ...
    circuit_manual_reordered = permute_detectors(
        circuit_manual, detector_positions.flatten()
    )
    ...
    assert np.array_equal(
        detector_positions_auto.flatten(), np.arange(len(detector_positions.flatten()))
    )
...
def test_pytree_list_with_detector_reorder():
    ...
    assert np.array_equal(np.array([int(d) for d in detector_list]), np.arange(3))
