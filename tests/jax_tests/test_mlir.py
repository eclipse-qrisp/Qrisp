...
def test_mlir_generation(lower_stablehlo):
    ...
    def main():
        ...
        [
            x(qv[i]) for i in range(1)
        ]  # Prepare Hartree-Fock state, H2O molecule has 10 electrons
...
def test_mlir_basic_dialect_operations(lower_stablehlo):
    ...
    assert "jasp.quantum_gate" in mlir_str or "jasp.gate" in mlir_str, (
        "gate operations not found in MLIR"
    )
...
def test_mlir_quantum_control_flow_rewriting(lower_stablehlo):
    ...
    assert "stablehlo.case" not in mlir_str, (
        "stablehlo.case should be rewritten to scf.if"
    )
    assert "stablehlo.while" not in mlir_str, (
        "stablehlo.while should be rewritten to scf.while"
    )
    assert "stablehlo.return" not in mlir_str, (
        "stablehlo.return should be rewritten to scf.yield"
    )
...
def test_mlir_qaoa(lower_stablehlo):
    ...
    assert "jasp.create_qubits" in mlir_str, (
        "MLIR should contain jasp.create_qubits operation"
    )
    assert "jasp.quantum_gate" in mlir_str, (
        "MLIR should contain jasp.quantum_gate operations"
    )
    ...
    assert "scf.while" in mlir_str, (
        "MLIR should contain SCF while for QAOA optimization loop"
    )
    assert "scf.yield" in mlir_str, "MLIR should contain SCF yield operations"

    # Verify that QAOA-specific operations are present
    assert "jasp.slice" in mlir_str or "jasp.get_qubit" in mlir_str, (
        "MLIR should contain qubit indexing operations"
    )
    ...
    if lower_stablehlo:
        ...
        assert "arith." in mlir_str or "linalg." in mlir_str, (
            "Lowered arith/linalg ops should be present"
        )
...
def test_mlir_array_operations(lower_stablehlo):
    ...
    assert "jasp.create_qubits" in mlir_str, "Should contain qubit creation"
    assert "jasp.slice" in mlir_str or "jasp.get_qubit" in mlir_str, (
        "Should contain array access operations"
    )
    assert "jasp.quantum_gate" in mlir_str or "jasp.gate" in mlir_str, (
        "Should contain quantum gates"
    )
...
def test_mlir_linalg_folding_after_stablehlo_lowering():
    ...
    mlir_str = str(xdsl_module)
    assert "linalg.generic" not in mlir_str, (
        "0-d linalg.generic should be folded after lowering StableHLO control flow"
    )
