...
"""Tests Jaspr-to-MLIR generation via to_mlir, covering StableHLO lowering and the JASP dialect."""
...
def test_mlir_qaoa(lower_stablehlo):
    ...
    def main():
        ...
        qaoa_maxcut = QAOAProblem(
            cost_operator=create_maxcut_cost_operator(G), mixer=RX_mixer, cl_cost_function=cl_cost
        )
...
def test_mlir_jasp_dialect_registration(lower_stablehlo):
    ...
    assert '!"jasp.QuantumState"' not in mlir_str, "QuantumState type is unregistered (printed with quotes)"
    assert '!"jasp.QubitArray"' not in mlir_str, "QubitArray type is unregistered (printed with quotes)"
    assert '!"jasp.Qubit"' not in mlir_str, "Qubit type is unregistered (printed with quotes)"
    assert "!jasp.QuantumState" in mlir_str, "QuantumState type not found in MLIR output"

    # Op names must appear without quotes
    assert '"jasp.create_qubits"' not in mlir_str, "create_qubits op is unregistered (printed with quotes)"
    assert '"jasp.quantum_gate"' not in mlir_str, "quantum_gate op is unregistered (printed with quotes)"
    assert '"jasp.measure"' not in mlir_str, "measure op is unregistered (printed with quotes)"
