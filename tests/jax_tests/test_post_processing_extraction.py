...
def test_bitstring_vs_array_equivalence():
    ...
    for bits in ["000", "001", "010", "011", "100", "101", "110", "111"]:
        ...
        assert result_array == result_bitstring, (
            f"Mismatch for {bits}: {result_array} != {result_bitstring}"
        )
...
def test_cond_primitive():
    ...
    )  # array[0]=qv[0]=False (extra elements ignored)
    assert result == 25, f"Expected 25, got {result}"

    # Test with meas=True -> true_branch(5) = 15
    result = post_proc(
        jnp.array([True, False])
    )  # array[0]=qv[0]=True (extra elements ignored)
...
def test_terminal_sampling_comparison():
    ...
    def compare_post_processed_distributions(
        qc_results, post_proc, terminal_results, tolerance=0.1
    ):
        """
        ...
        for key, prob in post_proc_dist.items():
            ...
            if terminal_key in terminal_results:
                ...
                if diff > tolerance:
                    return (
                        False,
                        f"Mismatch for {key}: post_proc={prob:.3f}, terminal={terminal_prob:.3f}",
                    )
            elif prob > tolerance:
                return (
                    False,
                    f"Unexpected post-processed outcome {key} with prob {prob:.3f}",
                )
    ...
    # Compare distributions
    match, msg = compare_post_processed_distributions(
        qc_results, post_proc, terminal_results
    )
    ...
    bits = bitstring_to_array(bitstring)
    result = post_proc(bits)
    assert result[0] == terminal_key[0], (
        f"qf1 mismatch: expected {terminal_key[0]}, got {result[0]}"
    )
    assert result[1] == terminal_key[1], (
        f"qf2 mismatch: expected {terminal_key[1]}, got {result[1]}"
    )
...
def test_parity_post_processing():
    ...
    result = post_proc(
        "10"
    )  # m1=True, m2=False -> parity=True, expectation=0 -> result=1
    assert result == 1, f"Expected 1 (mismatch indicator), got {result}"

    # Test matching expectation
    result = post_proc(
        "00"
    )  # m1=False, m2=False -> parity=False, expectation=0 -> result=0
    ...
    assert jnp.array_equal(result, jnp.array([1, 1, 1])), (
        f"Expected [1, 1, 1], got {result}"
    )

    # Test with all zeros
    result = post_proc("000000")
    assert jnp.array_equal(result, jnp.array([0, 0, 0])), (
        f"Expected [0, 0, 0], got {result}"
    )

    # Test jitting the scan-based post-processor
    jitted_scan_post_proc = jit(post_proc)
    result = jitted_scan_post_proc(
        jnp.array([True, False, True, False, True, False], dtype=bool)
    )
    assert jnp.array_equal(result, jnp.array([1, 1, 1])), (
        f"Expected [1, 1, 1] from jitted scan function, got {result}"
    )
    ...
    assert jnp.array_equal(result, jnp.array([1, 0, 1])), (
        f"Expected [1, 0, 1], got {result}"
    )

    # Test jitting the array parity post-processor
    jitted_array_post_proc = jit(post_proc)
    result = jitted_array_post_proc(
        jnp.array([False, False, False, False, False, False], dtype=bool)
    )
    assert jnp.array_equal(result, jnp.array([0, 0, 0])), (
        f"Expected [0, 0, 0] from jitted array function, got {result}"
    )
