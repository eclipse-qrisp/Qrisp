...
"""Tests for Qrisp's BlockEncoding using the CUDA-Q interface."""
...
def _post_selection(res_dict):
    """Post-select on the ancilla qubits being in the |0> state."""
...
def _assert_distributions_close(dict1, dict2, atol=1e-1):
    """Assert that two probability distributions (given as dictionaries) are close within a specified tolerance."""
    all_keys = set(dict1.keys()).union(set(dict2.keys()))
    for key in all_keys:
        val1 = dict1.get(key, 0.0)
        val2 = dict2.get(key, 0.0)
        assert np.isclose(val1, val2, atol=atol)


def _assert_cudaq_matches_jasp(main_func, shots_count=500, atol=1e-1):
    """Assert that the measurement distributions from CUDA-Q and JASP match after post-selection."""

    def wrapped_main_func():
        variables = main_func()
        measured_variables = [measure(var) for var in variables]
        return tuple(measured_variables)

    # Execute using CUDA-Q
    counts_cudaq = Counter(cudaq.run(cudaq_kernel(wrapped_main_func), shots_count=shots_count))
    res_dict_cudaq = {outcome: count / shots_count for outcome, count in counts_cudaq.items()}
    filtered_dict_cudaq = _post_selection(res_dict_cudaq)

    # Execute using JASP's terminal_sampling
    res_dict_jasp = terminal_sampling(main_func)()
    filtered_dict_jasp = _post_selection(res_dict_jasp)

    _assert_distributions_close(filtered_dict_cudaq, filtered_dict_jasp, atol=atol)
...
def test_simple_block_encoding_results(operator):
    ...
    _assert_cudaq_matches_jasp(main, shots_count=500, atol=1e-1)
