...
def test_parity_to_qc():
    ...
    assert isinstance(result, ParityHandle), (
        f"Expected ParityHandle, got {type(result)}"
    )

    # The quantum circuit should contain ONLY the measurements (parity tracked separately)
    assert len(qc.clbits) == 3, (
        f"Expected 3 classical bits (3 measurements), got {len(qc.clbits)}"
    )

    # Check the ParityHandle has 3 input clbits (stored directly)
    assert len(result.clbits) == 3, (
        f"Expected 3 input clbits in parity entry, got {len(result.clbits)}"
    )
    ...
    assert len(parity_ops) == 1, (
        f"Expected 1 ParityOperation in circuit, got {len(parity_ops)}"
    )
    ...
    assert isinstance(result, np.ndarray), (
        f"Expected numpy array for array parity, got {type(result)}"
    )
...
def test_measurement_array_basic():
    ...
    assert isinstance(result, Clbit), (
        f"Expected Clbit from sliced array, got {type(result)}"
    )
...
def test_measurement_array_reshape():
    ...
    assert isinstance(result, Clbit), (
        f"Expected Clbit after reshape, got {type(result)}"
    )
...
def test_measurement_array_concatenate_with_processed():
    ...
    assert isinstance(result[0], Clbit), (
        f"First element should be Clbit, got {type(result[0])}"
    )
...
def test_measurement_array_arithmetic():
    ...
    assert isinstance(result, np.ndarray), (
        f"Expected numpy array for mul, got {type(result)}"
    )
    assert all(isinstance(item, ProcessedMeasurement) for item in result.flat), (
...
def test_measurement_array_bitwise_ops():
    ...
    assert isinstance(result, np.ndarray), (
        f"Expected numpy array for AND, got {type(result)}"
    )
    assert all(isinstance(item, ProcessedMeasurement) for item in result.flat), (
    ...
    assert isinstance(result, np.ndarray), (
        f"Expected numpy array for OR, got {type(result)}"
    )
    assert all(isinstance(item, ProcessedMeasurement) for item in result.flat), (
    ...
    assert isinstance(result, np.ndarray), (
        f"Expected numpy array for XOR, got {type(result)}"
    )
    assert all(isinstance(item, ProcessedMeasurement) for item in result.flat), (
...
def test_measurement_array_chain_operations():
    ...
    assert isinstance(result, Clbit), (
        f"Expected Clbit from chained operations, got {type(result)}"
    )
    ...
    assert isinstance(result[1], Clbit), (
        f"Second element should be Clbit, got {type(result[1])}"
    )
...
def test_scatter_with_clbit_updates():
    ...
    assert len(unique_refs) == 6, (
        f"Expected 6 unique Clbit refs, got {len(unique_refs)}"
    )
    ...
    assert len(set(all_refs)) == 5, (
        f"Expected 5 unique Clbit refs total, got {len(set(all_refs))}"
    )
...
def test_quantum_array_measurement_with_parity():
    ...
    parity_ops = [op for op in qc.data if isinstance(op.op, ParityOperation)]
    assert len(parity_ops) == 2, f"Expected 2 ParityOperations, got {len(parity_ops)}"
...
def test_processed_parity_returns_processed_measurement():
    ...
    assert len(measure_ops) == 2, (
        f"Expected 2 measurement operations, got {len(measure_ops)}"
    )
...
def test_processed_measurement_in_control_raises_error():
    ...
    try:
        ...
        assert (
            "real-time feedback" in str(e).lower() or "processed" in str(e).lower()
        ), (
...
def test_nested_qache_with_measurements():
    ...
    assert result.shape == (3, N - 1), (
        f"Expected shape (3, {N - 1}), got {result.shape}"
    )

    # All elements should be Clbits
    for elem in result.flat:
        assert isinstance(elem, Clbit), f"Expected Clbit, got {type(elem)}"
