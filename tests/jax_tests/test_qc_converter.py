...
def test_measurement_array_reductions():
    ...
    if isinstance(result, np.ndarray):
        ...
        assert isinstance(result, ProcessedMeasurement), f"Expected ProcessedMeasurement for sum, got {type(result)}"
    ...
    if isinstance(result, np.ndarray):
        ...
        assert isinstance(result, ProcessedMeasurement), f"Expected ProcessedMeasurement for any, got {type(result)}"
...
def test_measurement_array_parity_with_processed():
    ...
    try:
        ...
        assert "processed" in str(e).lower(), f"Exception should mention 'processed', got: {e}"
...
def test_quantum_array_measurement_with_parity():
    ...
    assert len(qc.clbits) == 4, f"Expected 4 classical bits (4 measurements), got {len(qc.clbits)}"
    ...
    assert isinstance(result, np.ndarray), f"Parity result should be numpy array, got {type(result)}"
    assert result.shape == (3,), f"Expected shape (3,), got {result.shape}"

    # 6 measurements (parity tracked separately)
    assert len(qc.clbits) == 6, f"Expected 6 classical bits (6 measurements), got {len(qc.clbits)}"
...
def test_quantum_array_measurement_operations():
    ...
    assert isinstance(result, Clbit), f"Element access should return Clbit, got {type(result)}"
...
def test_nested_qache_with_measurements():
    ...
    measure_ops = [instr for instr in qc.data if instr.op.name == "measure"]
    assert len(measure_ops) == 3 * (N - 1), f"Expected {3 * (N - 1)} measurements, got {len(measure_ops)}"

    # Verify the circuit has reset operations
    reset_ops = [instr for instr in qc.data if instr.op.name == "reset"]
    assert len(reset_ops) == 3 * (N - 1), f"Expected {3 * (N - 1)} resets, got {len(reset_ops)}"
