...
import pytest
from qrisp.jasp.mlir.mlir_rewrites import (
    cmpi_extui_folding,
    scalar_linalg_folding,
    scalar_tensor_folding,
)
...
class TestFoldScalarLinalgGeneric:
    ...
    def build_test_ir(is_0d: bool = True, num_outputs: int = 1):
        ...
        func_op = func.FuncOp(
            "test_wrapper",
            ([tensor_type, tensor_type], [tensor_type] * num_outputs),
            Region([block]),
        )
    ...
    def test_no_folds(self, ctx, is_0d, num_outputs, reason):
        ...
        assert any(isinstance(op, linalg.GenericOp) for op in ops), (
            f"Failed on: {reason}"
        )
...
class TestCmpiExtUIFolding:
    ...
    def build_test_ir(pred_int: int, rhs_val: int, lhs_is_extui: bool = True):
        ...
        func_op = func.FuncOp(
            "test_wrapper", ([builtin.i1], [builtin.i1]), Region([block])
        )
    ...
    def test_all_fold_cases(self, ctx, pred_int, rhs_val, expected_result):
        ...
        if expected_result == "IDENTITY":
            ...
            true_consts = [
                ...
                and op.value.type == builtin.i1
                and op.value.value.data in [-1, 1]
            ]
            assert len(true_consts) >= 1, (
                "Expected a constant true (1 : i1) but found none."
            )
            ...
            false_consts = [
                ...
                if isinstance(op, arith.ConstantOp)
                and op.value.type == builtin.i1
                and op.value.value.data == 0
            ]
            assert len(false_consts) >= 1, (
                "Expected a constant false (0 : i1) but found none."
            )
    ...
    def test_no_folds(self, ctx, pred_int, rhs_val, lhs_is_extui, reason):
        ...
        module, block = self.build_test_ir(
            pred_int=pred_int, rhs_val=rhs_val, lhs_is_extui=lhs_is_extui
        )
        cmpi_extui_folding(ctx, module)
