...
"""Tests for the QUBO_problem QAOA solver in qrisp.qaoa."""
...
def test_QUBO():
    ...
    best_cost1, res_str = min(
        [(QUBO_obj(bitstring, Q1), bitstring) for bitstring in list(res.keys())], key=itemgetter(0)
    )
    ...
    res = QUBO_instance.run(qarg, depth=1, mes_kwargs={"backend": def_backend, "shots": 100000}, max_iter=50)
    best_cost2, res_str = max(
        [(QUBO_obj(bitstring, Q2), bitstring) for bitstring in list(res.keys())], key=itemgetter(0)
    )
