...
def test_mkcs_G1e2c():
    ...
    def G1e2c_onehot():
        # ONE-HOT
        qarg = QuantumArray(
            qtype=QuantumColor(color_list, one_hot_enc=True), shape=num_nodes
        )

        mkcs_1e2c = QAOAProblem(
            create_coloring_operator(G1e2c),
            apply_XY_mixer,
            create_coloring_cl_cost_function(G1e2c),
        )
        init_state = [random.choice(color_list) for _ in range(len(G1e2c))]

        mkcs_1e2c.set_init_function(lambda x: x.encode(init_state))
        res1e2c = mkcs_1e2c.run(
            qarg,
            depth,
            mes_kwargs={"backend": qaoa_backend, "shots": 100000},
            max_iter=25,
        )

        best_coloring, best_solution = min(
            [
                (mkcs_obj(quantumcolor_array, G1e2c), quantumcolor_array)
                for quantumcolor_array in res1e2c.keys()
            ],
            key=itemgetter(0),
        )
        ...
        best_coloring, res_str = min(
            [
                (mkcs_obj(quantumcolor_array, G1e2c), quantumcolor_array)
                for quantumcolor_array in list(res1e2c.keys())[:5]
            ],
            key=itemgetter(0),
        )
    ...
    def G1e2c_bin():
        qarg = QuantumArray(
            qtype=QuantumColor(color_list, one_hot_enc=False), shape=num_nodes
        )
...
def test_mkcs_5nodes():
    ...
    def G_onehot():
        ...
        best_coloring_onehot, best_solution_onehot = (
            mkcs_obj(res_str_onehot, G),
            res_str_onehot,
        )

        if all(
            res_str_onehot[pair[0]] != res_str_onehot[pair[1]]
            for pair in list(G.edges())
        ):
            return True
    ...
    def G_bin():
        ...
        mkcs_bin = QAOAProblem(
            create_coloring_operator(G), RX_mixer, create_coloring_cl_cost_function(G)
        )
        init_state = [random.choice(color_list) for _ in range(len(G))]

        mkcs_bin.set_init_function(lambda x: x.encode(init_state))

        res_bin = mkcs_bin.run(
            qarg, depth, mes_kwargs={"backend": qaoa_backend}, max_iter=25
        )
        ...
        best_coloring, best_solution = min(
            [
                (mkcs_obj(quantumcolor_array, G), quantumcolor_array)
                for quantumcolor_array in res_bin.keys()
            ],
            key=itemgetter(0),
        )

        best_coloring_bin, res_str_bin = min(
            [
                (mkcs_obj(quantumcolor_array, G), quantumcolor_array)
                for quantumcolor_array in list(res_bin.keys())[:5]
            ],
            key=itemgetter(0),
        )
        ...
        if all(
            res_str_bin[pair[0]] != res_str_bin[pair[1]] for pair in list(G.edges())
        ):
            return True
