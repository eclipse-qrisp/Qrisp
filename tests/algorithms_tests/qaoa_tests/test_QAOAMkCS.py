...
def test_mkcs_G1e2c():
    ...
    def G1e2c_onehot():
        ...
        best_coloring, best_solution = min(
            [(mkcs_obj(quantumcolor_array, G1e2c), quantumcolor_array) for quantumcolor_array in res1e2c.keys()],
            key=itemgetter(0),
        )
...
def test_mkcs_5nodes():
    ...
    def G_onehot():
        ...
        best_coloring, best_solution = min(
            [(mkcs_obj(quantumcolor_array, G), quantumcolor_array) for quantumcolor_array in res_onehot.keys()],
            key=itemgetter(0),
        )
    ...
    def G_bin():
        ...
        best_coloring, best_solution = min(
            [(mkcs_obj(quantumcolor_array, G), quantumcolor_array) for quantumcolor_array in res_bin.keys()],
            key=itemgetter(0),
        )

        best_coloring_bin, res_str_bin = min(
            [(mkcs_obj(quantumcolor_array, G), quantumcolor_array) for quantumcolor_array in list(res_bin.keys())[:5]],
            key=itemgetter(0),
        )
