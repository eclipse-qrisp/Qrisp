...
class TestCompressLayersBarriers:
    ...
    def test_gates_do_not_cross_a_barrier_on_their_own_qubit(self):
        ...
        names = _gate_names(compress_layers()(qc))
...
class TestCompressLayersParallelRendering:
    ...
    def test_layer_packs_into_the_minimum_number_of_columns(self, _stim):
        ...
        result = compress_layers(insert_barriers=True)(qc)
...
class TestCompressLayersInsertBarriers:
    ...
    def test_off_by_default(self):
        ...
        qc.h(0)
        qc.cx(0, 1)
        assert "barrier" not in _gate_names(compress_layers()(qc))
    ...
    def test_one_tick_per_layer(self, _stim):
        ...
        stim_circuit = compress_layers(insert_barriers=True)(qc).to_stim()
    ...
    def test_idempotent(self, _stim):
        ...
        once = compress_layers(insert_barriers=True)(qc)
        twice = compress_layers(insert_barriers=True)(once)
        assert _gate_names(once) == _gate_names(twice)
    ...
    def test_empty_circuit_gets_no_barrier(self):
        result = compress_layers(insert_barriers=True)(QuantumCircuit(3))
        assert len(result.data) == 0
    ...
    def test_measurement_statistics_preserved(self):
        ...
        qc.cx(1, 2)
        qc.measure(qc.qubits)
        assert compress_layers(insert_barriers=True).compare_measurement(qc)
...
class TestCompressLayersDetectorSemantics:
    ...
    def test_compress_layers_preserves_detector_count(self, _stim):
        qc = _rep_code_d3(rounds=2)
        expected = qc.to_stim().num_detectors
        assert compress_layers()(qc).to_stim().num_detectors == expected
        assert compress_layers(insert_barriers=True)(qc).to_stim().num_detectors == expected

    def test_compress_layers_keeps_detectors_silent(self, _stim):
        """The load-bearing assertion: a noiseless code must stay noiseless."""
        qc = _rep_code_d3(rounds=2)
        assert self._detection_events(compress_layers()(qc)) == 0
        assert self._detection_events(compress_layers(insert_barriers=True)(qc)) == 0
