...
def test_classical_bit_mapping():
    ...
    assert reordered_samples[:, 0].all(), (
        "Clbit 0 should always be True (qubit 1 with X)"
    )
    # Verify: qubit 2 had Y gate, so clbit 1 should always be True
    assert reordered_samples[:, 1].all(), (
        "Clbit 1 should always be True (qubit 2 with Y)"
    )
...
def test_stim_errors():
    ...
    qc.append(
        StimNoiseGate("HERALDED_PAULI_CHANNEL_1", 0.01, 0.02, 0.03, 0.04),
        [qc.qubits[0]],
    )
    ...
    qc.append(
        StimNoiseGate("ELSE_CORRELATED_ERROR", 0.2, pauli_string="ZZZ"), qc.qubits
    )
...
def test_detector_operation_conversion():
    ...
    stim_circuit, meas_map, det_map = qc.to_stim(
        return_measurement_map=True, return_detector_map=True
    )
...
def test_detector_permutation():
    ...
    if not np.array_equal(s_perm, reconstructed_perm):
        raise Exception(
            f"Deterministic sampler mismatch.\nOrig: {s_orig}\nPerm: {s_perm}\nExpP: {reconstructed_perm}"
        )
    print("Passed.")
    ...
    assert str_circ.strip().endswith("DETECTOR(0) rec[-2]"), (
        f"Circuit did not end with expected detector string. output: {str_circ.strip()[-20:]}"
    )
    ...
    for i, line in enumerate(lines):
        ...
        if "M 1" in line:
            m1_idx = i
        if "OBSERVABLE_INCLUDE(1)" in line:
            obs1_idx = i

    assert m0_idx < obs0_idx < m1_idx < obs1_idx, (
        f"Observables were moved or reordered improperly. Indices: M0={m0_idx}, Obs0={obs0_idx}, M1={m1_idx}, Obs1={obs1_idx}"
    )
...
def test_qc_parity_method_mixed_detectors_observables():
    ...
    det_handle = qc.parity(
        [qc.clbits[0], qc.clbits[1]], expectation=0, observable=False
    )
