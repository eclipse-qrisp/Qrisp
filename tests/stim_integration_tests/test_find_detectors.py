...
def test_noise_x_error_100_percent():
    ...
    for instruction in stim_circ:
        ...
        if instruction.name in ("M", "MR") and 1 in [
    ...
    assert samples_noiseless.sum() == 0, (
        "Noiseless circuit should have zero detector samples"
    )
    ...
    assert samples_noisy.sum() > 0, (
        "Noisy circuit should have non-zero detector samples"
    )
