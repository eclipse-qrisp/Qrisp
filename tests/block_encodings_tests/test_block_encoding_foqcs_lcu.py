...
def _heisenberg_from_def(L: int, g: NDArrayLike, J: NDArrayLike):
    ...
    for k, sigma in enumerate(sigma_list):
        ...
        for i in range(L):
            if i < L - 1:
                H = H + J[k] * np.kron(
                    np.identity(2**i), np.kron(sisj, np.identity(2 ** (L - i - 2)))
                )
            H = H + g[k] * np.kron(
                np.identity(2**i), np.kron(sigma, np.identity(2 ** (L - i - 1)))
            )
    return H
...
def _pick_ops_with_anc_all_zero(
    sv: NDArrayLike, anc: NDArrayLike, L: int
) -> NDArrayLike:
    res_ops = []
...
def test_foqcs_lcu_heisenberg_prep():
    ...
    for i in range(L - 1):
        dicke_2NN_double[2**i + 2 ** (i + 1) + 2 ** (i + L) + 2 ** (i + L + 1)] = (
            dicke_2NN_norm
        )
    ...
    def _ref_state_helper(coeff, q, param):
        ...
        return coeff * np.kron(
            [1 if i == 2 ** (6 - q) else 0 for i in range(2**6)], param
        )
...
def test_block_encoding_from_foqcs_lcu_heisenberg_prep():
    ...
    be = BlockEncoding.from_foqcs_lcu_prep(
        prep_r=prep_r, prep_l=prep_l, num_q_ops=L, norm=norm**2
    )
...
def test_foqcs_lcu_custom_prep_from_prep():
    ...
    be = BlockEncoding.from_foqcs_lcu_prep(
        prep_r=prep_r, prep_l=prep_l, num_q_ops=L, num_q_anc=n_anc_custom_prep
    )
...
def _compare_results(res_dict_1, res_dict_2, n):
    for k in range(2**n):
        ...
        assert np.isclose(val_1, val_2, atol=1e-6), (
            f"Mismatch at state |{k}>: {val_1} vs {val_2}"
        )
    ...
    for k in range(2**n1):
        for l in range(2**n2):
            ...
            assert np.isclose(val_be_kron, val_be1_be2), (
                f"Mismatch at state |{k}>: {val_be_kron} vs {val_be1_be2}"
            )
