...
"""Tests for the QSVT-based scaling factor of BlockEncoding.svt in qrisp.block_encodings."""
...
@pytest.mark.parametrize(
    "H1, H2, poly, parity",
    [
        (X(0) * X(1) + 0.2 * Y(0) * Y(1), Z(0) * Z(1) + X(2), np.array([1.0, 0.0, 1.0]), True),
        (0.5 * X(1) + 0.7 * Y(1) + 0.3 * X(4), Z(0) + Z(1) + X(2), np.array([0.0, 1.0, 0.0, 1.0]), False),
    ],
)
def test_block_encoding_svt_scaling(H1, H2, poly, parity):
    ...
    BE_svt = BE1.svt(poly, parity=parity_str) + BE2
