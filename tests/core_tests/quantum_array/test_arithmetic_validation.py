...
class TestArithmeticValidation:
    ...
    def test_validation_errors(self):
        ...
        with pytest.raises(
            TypeError, match="Element-wise operations require qtype 'QuantumFloat'"
        ):
        ...
        with pytest.raises(
            TypeError,
            match="Element-wise operations require both arrays to have qtype 'QuantumFloat'",
        ):
            qa_float._validate_arithmetic(qa_char)

        # Should raise TypeError for non-QuantumBool qtype of self
        with pytest.raises(
            TypeError, match="Element-wise operations require qtype 'QuantumBool'"
        ):
        ...
        with pytest.raises(
            TypeError,
            match="Element-wise operations require both arrays to have qtype 'QuantumBool'",
        ):
    ...
    def test_reduction_validation(self):
        ...
        with pytest.raises(
            TypeError, match="Reduction operation 'all' requires qtype 'QuantumBool'"
        ):
            qa_float.all()

        with pytest.raises(
            TypeError, match="Reduction operation 'any' requires qtype 'QuantumBool'"
        ):
            qa_float.any()
    ...
    def test_matmul_validation(self):
        ...
        with pytest.raises(
            ValueError, match="Incompatible shapes for matrix multiplication"
        ):
            qa1 @ qa2
        ...
        with pytest.raises(
            TypeError,
            match="Matrix multiplication requires qtype 'QuantumFloat' or 'QuantumModulus'",
        ):
