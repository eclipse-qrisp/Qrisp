...
"""Tests the quantum carry-lookahead adder (qcla) carry generation and addition for qq and cq cases."""
        ...
        for radix_exponent in [1, 0, 2]:
            for m in range(1, 6):
                for a in range(2**m):
                    ...
                    angles = np.angle(statevector_arr[np.abs(statevector_arr) > 1 / 2 ** ((c.size + b.size) / 2 + 1)])
