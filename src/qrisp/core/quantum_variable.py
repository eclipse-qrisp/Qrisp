...
def custom_qv(
    ...
    if not isinstance(labels, list):
        raise TypeError("Tried to create custom QuantumVariable without providing a list type")

    if len(labels) == 0:
        raise ValueError("Tried to create custom QuantumVariable without providing labels")
    if len(labels) == 1:
        n = 1
    else:
    ...
    class CustomQuantumVariable(QuantumVariable):
        """QuantumVariable subclass generated by custom_qv, decoding basis states via ``labels``."""
