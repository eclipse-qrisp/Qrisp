...
"""Benchmarks Qrisp against OpenFermion for Hamiltonian simulation resource counts and precision."""
...
def benchmark_hamiltonian_simulation(molecule_data):
    ...
    def count_rz(qc):
        ...
        return (
            ops_count_dic.get("rz", 0)
            + ops_count_dic.get("u1", 0)
            + 2 * ops_count_dic.get("u3", 0)
            + ops_count_dic.get("p", 0)
        )
    ...
    def count_cnot(qc):
        ...
        return (
            ops_count_dic.get("cx", 0)
            + ops_count_dic.get("cz", 0)
            + 3 * ops_count_dic.get("compiled_gidney_mcx", 0)
            + 0.5 * ops_count_dic.get("compiled_gidney_mcx_inv", 0)
        )
