...
ax1.plot(precisions[:n_qiskit], results["qiskit"]["cx_counts"], 
        marker='x', label='Qiskit #CX', 
        color=color2, linestyle='dashed', linewidth=2)
ax1.set_ylabel('CX Gate Count (log scale)', color=color2)
ax1.plot(precisions[:n_qiskit], results["qrisp"]["cx_counts"][:n_qiskit], 
        marker='x', label='Qrisp #CX', 
        color='#ff7f0e', linestyle='dashed', linewidth=2)
ax1.set_yscale('log')
...
ax2 = ax1.twinx()
color1 = '#6929C4'
ax2.plot(precisions, fit_data, marker='.', label="Extrapolation", fillstyle = "none", color = "#7d7d7d")
ax2.set_ylabel('Resource Computation Time (s, log scale)', color=color1)
ax2.plot(precisions[:n_qiskit], results["qiskit"]["times"], 
        marker='o', label='Qiskit Time', 
        color=color1, linestyle='solid', linewidth=3)
ax2.plot(precisions, results["qrisp"]["times"], 
        marker='o', label='Qrisp Time', 
        color='#20306f', linestyle='solid', linewidth=3)
ax2.set_yscale('log')
