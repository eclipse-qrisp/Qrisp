...
def create_max_indep_replacement_routine(res, problem_updated):
    ...
    problem_updated : List
            Updates that happened during the QIRO routine. Consists of the updated problem,
            a list of Qubits which were found to be positively correlated, i.e. part of the
            problem solution,
...
def create_max_indep_controlled_mixer_reduced(problem_updated):
    r"""Creates the ``controlled_RX_mixer`` for a QIRO instance of the maximal
    independent set problem for a given graph ``G`` following
    `Hadfield et al. <https://arxiv.org/abs/1709.03489>`_
...
def qiro_max_indep_set_init_function(solutions=[], exclusions=[]):
    r"""To be used for the controlled mixer approach of QIRO MIS. Only flips qubits
    which we found to be a part of the problem solution.
