...
def create_max_indep_set_mixer(G):
    r"""Creates the ``controlled_RX_mixer`` for an instance of the maximal independent
    set problem for a given graph ``G`` following
...
def create_max_indep_set_cl_cost_function(G):
    """Creates the classical cost function for an instance of the maximal independent
    set problem for a given graph ``G``.
