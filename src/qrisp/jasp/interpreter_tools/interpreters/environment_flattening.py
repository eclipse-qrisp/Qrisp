...
from qrisp.jasp.interpreter_tools import eval_jaxpr, extract_invalues, eval_jaxpr_with_context_dic, exec_eqn, reinterpret, ContextDict
...
@lru_cache(maxsize = int(1E5))
def flatten_collected_environments(jaspr):
...
def flatten_environment_eqn(env_eqn, context_dic):
    ...
    res = env_eqn.primitive.jcompile(env_eqn.params["jaspr"], *invalues)
    ...
    body_jaspr = env_eqn.params["jaspr"]
    
    from qrisp.environments import InversionEnvironment, ControlEnvironment
    from qrisp.jasp.environment_compilation import inv_transform

    # Perform the environment compilation logic
    if isinstance(env_eqn.primitive, InversionEnvironment):
        transformed_jaxpr = inv_transform(body_jaspr)
    elif isinstance(env_eqn.primitive, ControlEnvironment):
        num_ctrl = len(env_eqn.invars) - len(env_eqn.params["jaspr"].invars)
        transformed_jaxpr = body_jaspr.control(num_ctrl)
    else:
        
        transformed_jaxpr = body_jaspr
