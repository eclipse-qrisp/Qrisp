...
where
    ...
    free_indices     -- Jlist whose elements are qubit *positions* (integers)
                        inside pre_alloc_array that are currently unused
...
Jlist of integer positions into pre_alloc_array.
...
---------------------------------
``jasp.create_qubits``  -- pops N indices from free_indices into a fresh Jlist
                           that represents the new QubitArray.
...
AbstractQuantumState, AbstractQubitArray, and Jlist), no manual
    ...
    def evaluator(eqn, context_dic):
        if isinstance(eqn.primitive, QuantumPrimitive):
            ...
            if name == "jasp.create_quantum_kernel":
                ...
                free_indices = Jlist(jnp.arange(size)[::-1], max_size=size)
                insert_outvalues(
                ...
                _process_create_qubits(invars, outvars, context_dic)
    ...
    def transform(jaspr):
        ...
        for invar in jaspr.jaxpr.invars:
            if isinstance(invar, Literal):
                ...
                inner_args.append((
                    AbstractQubitArray(),
                    Jlist(jnp.arange(size)[::-1], max_size=size),
                    AbstractQuantumState(),
                ))
            elif isinstance(invar.aval, AbstractQubitArray):
                inner_args.append(Jlist())
        ...
        _initial_free_arr     = jnp.arange(size - 1, -1, -1, dtype=jnp.int64)
        _initial_free_counter = jnp.array(size, dtype=jnp.int64)

        def outer_fn(*args):
            # Expand every QuantumState arg to the 4 flat values expected by
            # the inner jaxpr: (pre_alloc_array, free_arr, free_counter, qst).
            inner_call_args = []
            for invar, arg in zip(jaspr.jaxpr.invars, args):
                if isinstance(invar.aval, AbstractQuantumState):
                    ...
                    inner_call_args.extend(
                        [pre_alloc, _initial_free_arr, _initial_free_counter, new_qst]
                    )
                else:
            ...
            #   (classical_outs...,
            #    [pre_alloc_array, jlist_arr, jlist_counter, QuantumState] * n)
            # Each QuantumState outvar was expanded to 4 values by the inner
            # trace (QubitArray + Jlist.array + Jlist.counter + QuantumState).
            n_classical   = len(inner_result) - 4 * qst_outvar_count
            classical_outs = list(inner_result[:n_classical])

            final_qsts = []
            for i in range(qst_outvar_count):
                base          = n_classical + i * 4
                pre_alloc_out = inner_result[base]        # QubitArray
                abs_qst_out   = inner_result[base + 3]    # QuantumState
...
def _process_create_qubits(invars, outvars, context_dic):
    """Pop ``size`` indices from the free pool into a new Jlist (QubitArray)."""
    ...
    # Ensure size is a JAX tracer so fori_loop can trace through it.
    @jit
    def _make_tracer(x):
        return x

    size = _make_tracer(size)
...
def _process_get_qubit(invars, outvars, context_dic):
    """Index into the Jlist to retrieve the integer position of a qubit."""
...
def _process_fuse(eqn, context_dic):
    """Merge two Jlists (or a Jlist and a single qubit position) into one."""
...
def _exec_multi_measurement(pre_alloc_array, qubit_list, abs_qst):
    ...
    """
    static_qubit_array = qubit_list.array
...
def _process_while(eqn, context_dic, evaluator):
    ...
    and Jlist qubit arrays.  ``jax.lax.while_loop`` handles these because both
    AbstractQuantumState and Jlist are registered JAX pytrees.
    """
    invalues = extract_invalues(eqn, context_dic)
