...
@lru_cache(int(1e5))
def get_compiled_profiler_new(jaxpr, metric_cls, zipped_profiling_dic, meas_behavior):

    profiling_dic = dict(zipped_profiling_dic)
    metric = metric_cls(profiling_dic, meas_behavior)

    profiling_eqn_evaluator = make_profiling_eqn_evaluator_new(metric)
...
def make_profiling_eqn_evaluator_new(metric) -> Callable:
    ...
    def profiling_eqn_evaluator(eqn, context_dic):
        ...
        if isinstance(eqn.primitive, QuantumPrimitive):
            ...
            profiler = get_compiled_profiler_new(
                eqn.params["jaxpr"],
