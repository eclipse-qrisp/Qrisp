...
from xdsl.dialects.builtin import i1, TensorType, IntegerAttr, i64
...
# ===================================================================
# CUDA-Q Backend Bug Bypass
# ===================================================================

def _apply_cudaq_while_hotfix(block: Block) -> None:
    """
    Hotfix for a known CUDA-Q compiler bug where inclusive loop bounds 
    in `cc.loop` while-conditions (`sge`) cause premature termination.
    
    We rewrite:
        `A sge B` (>=)  -->  `A sgt B-1`
    """
    # Dynamically fetch the proper xDSL classes across different versions
    ConstClass = getattr(arith, "Constant", getattr(arith, "ConstantOp", None))
    XoriClass = getattr(arith, "XOrIOp", getattr(arith, "XoriOp", None))
    
    for op in list(block.ops):
        if op.name == "arith.cmpi":
            # 1. Try direct property access (Modern xDSL)
            pred_attr = getattr(op, "predicate", None)
            
            # 2. Try properties dictionary (Transitional xDSL)
            if pred_attr is None and hasattr(op, "properties"):
                pred_attr = op.properties.get("predicate")
                
            # 3. Try attributes dictionary (Older xDSL)
            ...
            # Extract the actual value (could be IntegerAttr, raw int, or Enum)
            val = getattr(pred_attr, "value", pred_attr)
            pred_val = getattr(val, "data", val)
            
            # Convert to string to safely catch xDSL IntEnums (e.g., CmpiPredicate.sge)
            ...
            if new_pred:
                ...
                # 1. Evaluate strict condition (which CUDA-Q handles correctly)
                new_cmp = arith.CmpiOp(lhs, rhs_new, new_pred)
