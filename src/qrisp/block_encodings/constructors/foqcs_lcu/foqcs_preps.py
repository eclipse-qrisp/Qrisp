...
def foqcs_prep_spin_glass(
    ...
    J_arr = {
        dim: [np.asarray(J[dim][k - 1], dtype=complex) for k in range(1, L)]
    ...
    if subprep_norm <= _FOQCS_SPIN_GLASS_TOL:
        raise ValueError(
            "Cannot prepare spin-glass PREP state: all coefficients are zero."
        )
    ...
    for k in range(1, L):
        with control(prep_qv[k]):
            _phase_fix_dicke1_unbalanced(
                prep_qv[fh1 : fh1 + L - k], J_arr["X"][k - 1], cutoff[0][k]
            )

        with control(prep_qv[L + k]):
            _phase_fix_dicke1_unbalanced(
                prep_qv[fh1 : fh1 + L - k], J_arr["Y"][k - 1], cutoff[1][k]
            )

        with control(prep_qv[2 * L + k]):
            _phase_fix_dicke1_unbalanced(
                prep_qv[fh2 : fh2 + L - k], J_arr["Z"][k - 1], cutoff[2][k]
            )
