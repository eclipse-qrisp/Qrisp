...
class BackendServer:
    """
    This class allows convenient setup of a server respecting the
    `Qunicorn<https://qunicorn-core.readthedocs.io/en/latest/index.html>`_ interface.
    ...
    def __init__(self, run_func, ip_address=None, port=None):
        ...
        def deployments():
            ...
            response = {"id": len(self.deployments) - 1, "deployedBy": {
                "id": 0,
                "name": ""
            }, "deployedAt": str(date.today()), "name": "", "programs": data["programs"]}
        ...
        def jobs_post():
            ...
            job_dic = {"deploymentId": deployment_id, "counts": [], "shots": data["shots"], "exception": None,
                       "quantumCircuits": deployment["programs"], "run_thread": run_thread,
                       "start_time": str(datetime.now()), "end_time": "-"}
        ...
        def get_job(job_id):
            ...
            # Find out weather the job finished
            ...
            # Find out wether the job succeeded
    ...
    def start(self):
        ...
        import requests
        # Hold program until the server answers
        while True:
            try:
                requests.get("http://" + self.ip_address + ":" + str(self.port) + "/jobs/0/")
